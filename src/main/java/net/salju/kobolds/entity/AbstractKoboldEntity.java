...
public abstract class AbstractKoboldEntity extends Monster implements CrossbowAttackMob, RangedAttackMob {
	...
	public InteractionResult mobInteract(Player player, InteractionHand hand) {
		...
		if (this.isAlive()) {
			if (player.getMainHandItem().isEmpty() && player.getOffhandItem().isEmpty()) {
			...
			} else if (!world.isClientSide() && off.isEmpty() && this.isEffectiveAi()) {
				...
				} else if (gem.getItem() == KoboldsItems.KOBOLD_SPAWN_EGG.get()) {
					if (this instanceof Kobold) {
						if (this.level() instanceof ServerLevel lvl) {
	...
	public static List<ItemStack> getTradeItems(AbstractKoboldEntity kobold, String table) {
		LootTable loot = kobold.level().getServer().getLootData().getLootTable(new ResourceLocation(table));
		List<ItemStack> list = loot.getRandomItems((new LootParams.Builder((ServerLevel) kobold.level())).withParameter(LootContextParams.THIS_ENTITY, kobold).create(LootContextParamSets.EMPTY));
		return list;
	}
